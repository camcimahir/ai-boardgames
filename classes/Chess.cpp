...
void Chess::bitMovedFromTo(Bit &bit, BitHolder &src, BitHolder &dst)
{
    ...
    // record this move in the history so the opening book can recognise the line
    _moveHistory.emplace_back(fromIndex, toIndex);
...
// Opening book
// -----------------------------------------------------------------------------
// The book covers the first 5 AI (Black) moves of a handful of classical lines.
// Each line is the full sequence of (fromSquare, toSquare) pairs, alternating
// White, Black, White, Black, ...  Square index is rank * 8 + file
// (a1 = 0, h1 = 7, a8 = 56, h8 = 63).
// At lookup time we compare the move history played so far against the prefix
// of every line; if any line matches, we play the next move from that line.
// =============================================================================
namespace {
struct OpeningLine {
    const char* name;
...
// Helper: file 'a'..'h' (0..7), rank 1..8 (1..8) -> square index
constexpr int sq(int file, int rank) { return (rank - 1) * 8 + file; }

const std::vector<OpeningLine>& openingBook()
{
    static const std::vector<OpeningLine> book = {
        // Sicilian Najdorf:  1.e4 c5  2.Nf3 d6  3.d4 cxd4  4.Nxd4 Nf6  5.Nc3 a6
        { "Sicilian Najdorf", {
            {sq(4,2), sq(4,4)}, {sq(2,7), sq(2,5)},
            {sq(6,1), sq(5,3)}, {sq(3,7), sq(3,6)},
            {sq(3,2), sq(3,4)}, {sq(2,5), sq(3,4)},
            {sq(5,3), sq(3,4)}, {sq(6,8), sq(5,6)},
            {sq(1,1), sq(2,3)}, {sq(0,7), sq(0,6)},
        }},
        // Nimzo-Indian (Hubner):  1.d4 Nf6  2.c4 e6  3.Nc3 Bb4  4.e3 c5  5.Bd3 d5
        { "Nimzo-Indian (Hubner)", {
            {sq(3,2), sq(3,4)}, {sq(6,8), sq(5,6)},
            {sq(2,2), sq(2,4)}, {sq(4,7), sq(4,6)},
            {sq(1,1), sq(2,3)}, {sq(5,8), sq(1,4)},
            {sq(4,2), sq(4,3)}, {sq(2,7), sq(2,5)},
            {sq(5,1), sq(3,3)}, {sq(3,7), sq(3,5)},
        }},
        // Queen's Gambit Declined:  1.Nf3 d5  2.d4 Nf6  3.c4 e6  4.Nc3 Be7  5.Bg5 h6
        { "Queen's Gambit Declined", {
            {sq(6,1), sq(5,3)}, {sq(3,7), sq(3,5)},
            {sq(3,2), sq(3,4)}, {sq(6,8), sq(5,6)},
            {sq(2,2), sq(2,4)}, {sq(4,7), sq(4,6)},
            {sq(1,1), sq(2,3)}, {sq(5,8), sq(4,7)},
            {sq(2,1), sq(6,5)}, {sq(7,7), sq(7,6)},
        }},
        // English (Reversed Sicilian):  1.c4 e5  2.Nc3 Nf6  3.Nf3 Nc6  4.g3 d5  5.cxd5 Nxd5
        { "English Symmetrical", {
            {sq(2,2), sq(2,4)}, {sq(4,7), sq(4,5)},
            {sq(1,1), sq(2,3)}, {sq(6,8), sq(5,6)},
            {sq(6,1), sq(5,3)}, {sq(1,8), sq(2,6)},
            {sq(6,2), sq(6,3)}, {sq(3,7), sq(3,5)},
            {sq(2,4), sq(3,5)}, {sq(5,6), sq(3,5)},
        }},
    };
    return book;
}
...
std::optional<std::pair<int,int>> Chess::findBookMove() const
{
    const std::size_t historySize = _moveHistory.size();
    for (const auto& line : openingBook()) {
        if (line.moves.size() <= historySize) continue;       // book line exhausted
        bool matches = true;
        for (std::size_t i = 0; i < historySize; ++i) {
            if (line.moves[i] != _moveHistory[i]) {
                matches = false;
                break;
            }
        }
        if (matches) {
            std::cout << "[Book] Following \"" << line.name << "\" line." << std::endl;
            return line.moves[historySize];
        }
    }
    return std::nullopt;
}
...
void Chess::updateAI() {
    ...
    // 1) Try the opening book first. If the game so far matches a known
    //    classical line, play the prepared move and skip search entirely.
    if (auto bookMove = findBookMove()) {
        int srcSquare = bookMove->first;
        int dstSquare = bookMove->second;
        ...
        // If we somehow can't apply the book move (e.g. board state diverged
        // from the line), fall through to negamax.
        std::cout << "[Book] Book move unplayable, falling back to negamax." << std::endl;
    }

    // 2) Out of book: normal negamax search.
    GameState gs;
    ...
    for (auto& move : moves) {
        gs.pushMove(move);
        int score = -negamax(gs, 4, -INF, INF, -playerColor);
        gs.popMove();
        if (score > bestScore) {
            bestScore = score;
            bestMove = move;
