...
private:
    ...
    // Opening book: returns the next AI move (from, to) if the game so far
    // matches a known classical line, otherwise std::nullopt.
    std::optional<std::pair<int,int>> findBookMove() const;
    ...
    // Move history (every move played by either side as (fromSq, toSq)).
    // Used by the opening book to recognise known classical lines.
    std::vector<std::pair<int,int>> _moveHistory;
};
